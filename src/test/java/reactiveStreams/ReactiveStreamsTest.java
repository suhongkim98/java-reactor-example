 ...
 * 명령형 코드는 작업이 수행되는 동안, 특히 원격지 서버로부터 데이터베이스에 데이터를 쓰거나 가져오는 것이나 원격지의 API call을 하는 등과 같은 동기식 작업을 처리하는 것이라면 이 작업이 완료될 때까지 그것을 처리하던 스레드는 아무 것도 할 수 없게된다.
 * 따라서 동기식 작업을 수행하던 스레드는 waiting 상태로 전환되며 이렇게 차단되는 스레드는 낭비다.
 * 리액티브 프로그래밍의 경우 순차적으로 수행되는 작업 단계를 나타낸 것이 아니라 데이터가 흘러가는 파이프라인이나 스트림을 표현한다.
 * 리액티브 스트림즈에서 스레드는 명령헝 프로그래밍처럼 동기식 작업을 처리하는 동안 waiting 상태로 전환되지 않고 처리 가능한 데이터가 있을 때마다 받아 처리하여 성능을 쥐어짜낸다.
 * <p>
